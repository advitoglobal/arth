...
export function EnquiryRow({
  ...
  return (
    <div className="grid grid-cols-1 gap-2 border-b border-[var(--arth-n10)] px-4 py-3 lg:grid-cols-9 lg:items-center">
      <div>
        <p className="font-semibold">{row.customer_name}</p>
        <p className="font-data text-[12.5px] text-[var(--arth-n60)]">{row.phone}</p>
      </div>
      <div>
        <p>{row.model_interest}</p>
        <p className="text-[12.5px] text-[var(--arth-n60)]">{row.variant_interest}</p>
      </div>
      <div>
        <p>{row.source_key}</p>
        <p className="text-[12.5px] text-[var(--arth-n60)]">{row.source_detail}</p>
      </div>
      <div>
        <p>{row.stage_label}</p>
      ...
      <div className="text-[12.5px]">{eventDate(row)}</div>
      <div
        className={
          next.overdue
            ? "font-semibold text-[var(--arth-overdue)]"
            : "font-medium"
        }
      >
        {next.text}
      </div>
      <div>{showBand ? row.difficulty_band : ""}</div>
      <div className="arth-num font-data">{inr(Number(row.expected_value_paise) / 100)}</div>
...
export function RowHead() {
  return (
    <div className="hidden grid-cols-9 gap-2 border-b border-[var(--arth-ink)] px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--arth-slate)] lg:grid">
      <span>Customer</span>
      <span>Vehicle</span>
      <span>Source</span>
      <span>Stage</span>
