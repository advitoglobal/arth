...
async function inTenant<T>(tenantId: string, fn: (tx: Tx) => Promise<T>) {
...
async function main() {
  const wf = await inTenant(WHITEFIELD, (tx) => searchEnquiries(tx, { q: "0001" }));
  if (!wf.some((r) => r.customer_name === "Ramesh Kumar")) {
    throw new Error("Whitefield search 0001 should find Ramesh Kumar");
  }

  const byName = await inTenant(WHITEFIELD, (tx) => searchEnquiries(tx, { q: "Ramesh" }));
  if (!byName.some((r) => r.customer_name === "Ramesh Kumar")) {
    throw new Error("Name search should find Ramesh Kumar");
  }

  const byModel = await inTenant(WHITEFIELD, (tx) => searchEnquiries(tx, { q: "Brezza" }));
  ...
  const byEnquiryNo = await inTenant(WHITEFIELD, (tx) => searchEnquiries(tx, { q: no }));
  ...
  const namedGoogle = await inTenant(WHITEFIELD, (tx) =>
  ...
  const parked = await inTenant(WHITEFIELD, (tx) => searchEnquiries(tx, { parked: "yes" }));
  ...
  const arrived = await inTenant(WHITEFIELD, (tx) =>
  ...
  const missPhone = await inTenant(WHITEFIELD, (tx) => searchEnquiries(tx, { q: "9999888877" }));
  if (missPhone.length !== 0) {
    throw new Error("Unknown phone should return no rows so File this enquiry can show");
  }

  const coastal = await inTenant(COASTAL, (tx) => searchEnquiries(tx, { q: "0001" }));
